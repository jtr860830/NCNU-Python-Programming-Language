...
pygame.init()
score = 0  # 得分
font = pygame.font.Font("./NotoSansTC-Regular.otf", 20)  # 下方訊息字體
font1 = pygame.font.Font("./NotoSansTC-Regular.otf", 32)  # 結束程式訊息字體
soundhit = pygame.mixer.Sound("./1.mp3")  # 接到磚塊音效
soundpad = pygame.mixer.Sound("./2.mp3")  # 接到滑板音效
...
red_ball = Ball(5, 300, 350, 10, (255, 0, 0))  # 建立紅色球物件
green_ball = Ball(8, 300, 350, 10, (0, 255, 0))
allsprite.add(red_ball)  # 加入全部角色群組
allsprite.add(green_ball)
pad = Pad()  # 建立滑板球物件
...
while running:
    ...
    if playing == True:  # 遊戲進行中
        ...
        red_fail = red_ball.update()  # 移動球體
        blue_fail = green_ball.update()  # 移動球體
        if red_fail and blue_fail:  # 球出界
            gameover("失敗，再接再勵！")
        pad.update()  # 更新滑板位置
        hitbrick = pygame.sprite.spritecollide(red_ball, bricks, True)  # 檢查球和磚塊碰撞
        if len(hitbrick) > 0:  # 球和磚塊發生碰撞
            score += len(hitbrick)  # 計算分數
            soundhit.play()  # 球撞磚塊聲
            red_ball.rect.y += 20  # 球向下移
            red_ball.bounceup()  # 球反彈
            if len(bricks) == 0:  # 所有磚塊消失
                gameover("恭喜，挑戰成功！")
        hitbrick = pygame.sprite.spritecollide(green_ball, bricks, True)  # 檢查球和磚塊碰撞
        if len(hitbrick) > 0:  # 球和磚塊發生碰撞
            score += len(hitbrick)  # 計算分數
            soundhit.play()  # 球撞磚塊聲
            green_ball.rect.y += 20  # 球向下移
            green_ball.bounceup()  # 球反彈
            if len(bricks) == 0:  # 所有磚塊消失
                gameover("恭喜，挑戰成功！")
        hitpad = pygame.sprite.collide_rect(red_ball, pad)  # 檢查球和滑板碰撞
        if hitpad:  # 球和滑板發生碰撞
            soundpad.play()  # 球撞滑板聲
            red_ball.bounceup()  # 球反彈
        hitpad = pygame.sprite.collide_rect(green_ball, pad)  # 檢查球和滑板碰撞
        if hitpad:  # 球和滑板發生碰撞
            soundpad.play()  # 球撞滑板聲
            green_ball.bounceup()  # 球反彈
